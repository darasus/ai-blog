export const cooking = [
  "30 easy and healthy dinner recipes",
  "How to make the perfect roast chicken",
  "The ultimate guide to making a perfect pie crust",
  "From soup to nuts - the best recipes for winter",
  "One pot wonders - easy recipes for busy weeknights",
  "Comfort food recipes to warm your soul",
  "5 easy tips for making better pasta dishes",
  "The best way to cook eggs - scrambled, poached or sunny side up?",
  "How to make the perfect grilled cheese sandwich",
  "10 ways to use up leftover cooked rice",
  "Make your own - recipes for DIY condiments and sauces",
  "The best recipes for using up seasonal produce",
  "5 simple tips for making better salads",
  "One-dish meals - recipes for busy weeknights",
  "How to make the perfect omelette",
  "The ultimate guide to making pizzas at home",
  "Sourdough starter - how to make your own",
  "5 easy tips for making better soup",
  "3 recipes for homemade bread that's better than store-bought",
  "5 ways to cook with tofu - for meat-eaters and vegetarians",
  "The best recipes for using up leftover cooked",
  "The Best Foods to Eat for Weight Loss",
  "The Top 10 Healthiest Foods to Eat",
  "The 10 Worst Foods to Eat if You're Trying to Lose Weight",
  "5 Simple Tips for Eating healthier Every Day",
  "6 Ways To Make Your Diet More nutritious ",
  "The Truth About carbs - good and bad",
  "How To Cut Out processed foods From Your Diet",
  "3 Steps To Spring Clean Your Eating habits",
  "5 Unexpected benefits of a Plant based diet",
  "Why salmon should be a weekly staple",
  "Meal Prepping On A Budget- tips and tricks",
  "The Benefits Of Juicing: 4 Recipes To Get You Started",
  "Teaching kids about nourishment- fun and easy recipes",
  "The Environmental impact of our food choices",
  "Plant proteins vs animal proteins",
  "Is Organic Really worth the extra cost?",
  "Farm Fresh produce - where to find it & how it impacts your health",
  "What are fermented foods & why you should start eating them",
  "Need some recipe inspiration? try these",
  "Feeding a family on a budget - tips and recipes",
  "When you're short on time: 30 Minute meals or less",
  "Learning how to cook can save your life",
  "|Following food trends - is it worth it?",
  "Could veganism be the answer to climate change?",
  "Why you should add more greens into your diet",
  "Nutrient dense superfoods that improve overall health",
  "Common myths about organic products",
  "Smart waysahan help reduce grocery bills while also eating healthy",
  "Choosing seasonal fruits + vegetables",
  "Make ahead freezer meals for busy families",
  "Protein packed snacks that Power through any afternoon slump",
  "Breakfast ideas beyond cereal + oatmeal",
  "Rising popularity of meatless Mondays",
  "Slow cooker suppers for chilly evenings",
  "One pan dinners = fewer dishes",
  "40 easy Crockpot Dinners For Busy Weeknights",
  "Instant Pot Ideas For Beginners",
  "Quick weeknight pasta recipes",
  "Creative Ways Hanover Use Leftover Easter Eggs",
  "Decadent Desserts That Won't Derail Your Diets",
  "Breakfast Ideas That Will Make mornings Infinitely Better 50 Packable Lunch Ideas For Work Or School",
];
